...
class _TaskFilterState extends State<TaskFilter> {
  ...
  void initState() {
    ...
    for (var item in tasks) {
      if(item['profession_id']==widget.professionId)
      tasksList.add(item);
    }
  }
  @override
        ...
        itemBuilder: (context, index){
          return Padding(
              ...
              child: GestureDetector(
                onTap: (){
                  Navigator.of(context).push(MaterialPageRoute(builder: (_)=>OrderListing(taskId: tasksList[index]['id'])));
                },
                child: SizedBox(
                  height: 50,
                  child: DecoratedBox(
                    ...
                    child: Center(child: Text(tasksList[index]['name'], style: AppFonts.normalPrimaryWhite)),
                  )
                ),
              )
            );
